...
def gen_test(outdir, num_files=2500):
...
def gen_scons(outdir, num_files):
    ...
    sfile.write("VariantDir('buildscons', '.', duplicate=0)\n")
    sfile.write("env=Environment()\n")
    sfile.write("""src_files = Split('main.c""")
    for i in range(num_files):
        sfile.write(""" file%d.c""" % i)
    sfile.write("""')\n""")
    sfile.write("""env.Program('buildscons/speedtest', source=['buildscons/' + x for x in src_files])\n""")
...
def gen_bazel(outdir, num_files):
    ws_file = open(os.path.join(outdir, '..', 'WORKSPACE'), 'w')
    ws_file.write('\n')
    build_file = open(os.path.join(outdir, 'BUILD'), 'w')
    build_file.write('''
cc_binary(
    name = "speedtest_bzl",
    srcs = ["main.c"],
